...
void PJSIPManager::set_transport_type(TransportType type) {
  if (initialized_) {
    std::cout << "[PJSIP] Cannot change transport after initialization"
              << std::endl;
    return;
  }
...
void PJSIPManager::init() {
  ...
  // Do NOT force audio settings; let CoreAudio use its defaults for best
  // compatibility
  ep_->libInit(ep_cfg);
...
void PJSIPManager::set_ice_enabled(bool enabled) { ice_enabled_ = enabled; }
void PJSIPManager::set_interface_ip(const std::string &ip) {
  interface_ip_ = ip;
}

void PJSIPManager::register_account(const std::string &uri,
  ...
  acfg.sipConfig.transportId = transport_id_;
    ...
    std::cout << "[PJSIP] Note: interface_ip is set but will not override SIP "
              ...
              << std::endl;
    // Optionally set outbound proxy if you **really** know what you're doing
  ...
  std::cout << "[PJSIP] Registering account: " << uri << ", user: " << username
            << std::endl;
  if (ice_enabled_) {
    std::cout << "[PJSIP] ICE is enabled. STUN and candidate negotiation will "
                 "be used."
              << std::endl;
  } else {
    std::cout << "[PJSIP] ICE is disabled. Using direct interface routing."
              << std::endl;
  }
}
...
void PJSIPManager::call(int acc_id, const std::string &destination) {
  ...
  // (PJSUA2 does not expose direct per-call ICE toggle, but account setting
  // applies)
...
void MyAccount::onIncomingCall(OnIncomingCallParam &iprm) {
  ...
  if (mgr_) {
    ...
    std::cout << "Type 'accept " << iprm.callId << "' to answer or 'reject "
              << iprm.callId << "' to decline." << std::endl;
  }
}
...
void MyCall::onCallState(OnCallStateParam &prm) {
  CallInfo ci = getInfo();
  std::cout << "[PJSIP] Call: " << ci.remoteUri << " [" << ci.stateText
            << "] (state=" << ci.state << ")" << std::endl;
  std::cout << "  Call ID: " << ci.id << ", State code: " << ci.state
            << ", Last status code: " << ci.lastStatusCode << std::endl;
  if (ci.state == PJSIP_INV_STATE_CALLING) {
    std::cout << "  [PJSIP] Outgoing call, waiting for response from remote..."
              << std::endl;
  } else if (ci.state == PJSIP_INV_STATE_EARLY) {
    std::cout << "  [PJSIP] Call is ringing or in early media. Still waiting "
                 "for answer..."
              << std::endl;
  } else if (ci.state == PJSIP_INV_STATE_CONNECTING) {
    std::cout << "  [PJSIP] Call is connecting, waiting for ACK..."
              << std::endl;
  } else if (ci.state == PJSIP_INV_STATE_CONFIRMED) {
    std::cout
        << "  [PJSIP] Call is confirmed (ACK received). Call is established!"
        << std::endl;
...
void MyCall::onCallMediaState(OnCallMediaStateParam &prm) {
  ...
  if (ci.media.size() > 0 && ci.media[0].type == PJMEDIA_TYPE_AUDIO &&
    ...
    try {
      ...
      Endpoint::instance().audDevManager().getCaptureDevMedia().startTransmit(
          aud_med);
      std::cout << "[PJSIP] Audio media active (call_id=" << ci.id << ")"
                << std::endl;
      std::cout << "  Media count: " << ci.media.size()
                << ", Media status: " << ci.media[0].status << std::endl;
    } catch (Error &err) {
      std::cout << "[PJSIP] Media error: " << err.info() << std::endl;
    }
  } else {
    std::cout << "[PJSIP] Media not active or not audio (call_id=" << ci.id
              << ")" << std::endl;
  }
}
